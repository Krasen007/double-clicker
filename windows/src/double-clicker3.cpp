...
//TODO: Fix key selection
int selectSingleClickKey = VK_F9;
int selectDoubleClickKey = VK_F10;

const int singleClickVKey = selectSingleClickKey; //VK_F9;
const int doubleClickVKey = selectDoubleClickKey; //VK_F10;

static INPUT g_mouseLeftClick[2];
static INPUT g_mouseDoubleLeftClick[4];

static TCHAR g_prgInfoMsg[1024] = _T("Double Clicker\n");
...
static BOOL RegisterHotKeys()
{
	...
	{
		_stprintf_s(buf, 1024, _T("Couldn't assign F%d key for single-click"), singleClickVKey - VK_F1 + 1);
		OutputDebugString(buf);
		_tcscat_s(g_prgInfoMsg, _countof(g_prgInfoMsg), buf);
	}
	...
	{
		_stprintf_s(buf, 1024, _T("Couldn't assign F%d key for double-click"), doubleClickVKey - VK_F1 + 1);
		OutputDebugString(buf);
		_tcscat_s(g_prgInfoMsg, _countof(g_prgInfoMsg), buf);
	}
